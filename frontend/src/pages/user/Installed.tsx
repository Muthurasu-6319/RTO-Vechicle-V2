...
const Installed = () => {
  ...
  const handleDownload = async (app: any) => {
    ...
    try {
      ...
      // Step 1: Request presigned download URL from backend for temp certificate
      try {
        const reqUrl = appId 
          ? `${backendUrl}/api/applications/${appId}/download-certificate?type=temp${urlQuery}`
          : `${backendUrl}/api/applications/download-certificate?type=temp${urlQuery}`;
      ...
      const downloadProxyUrl = `${backendUrl}/api/download-proxy?${proxyId}type=temp&filename=${encodeURIComponent(filename)}${urlQuery}`;
      const a = document.createElement('a');
